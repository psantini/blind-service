-- Allow scoring_type = 'none' for unscored (free-response) questions

-- 1. Widen the check constraint
alter table public.attributes
  drop constraint if exists attributes_scoring_type_check;

alter table public.attributes
  add constraint attributes_scoring_type_check
  check (scoring_type in ('exact', 'bracket', 'none'));

-- 2. Update scoring function to skip 'none' attributes
