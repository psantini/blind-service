...
import { createGroup, deleteGroup, createInvite, revokeInvite, addMember, removeMember, updateMemberRole } from './actions';
...
export default async function GroupsPage() {
  ...
  // Super admins see all groups; group admins see only groups they manage
  const { data: managedMemberships } = isSuperAdmin
    ? { data: null }
    : await adminClient
        .from('group_members')
        .select('group_id')
  ...
  if (!isSuperAdmin && (managedMemberships ?? []).length === 0) redirect('/dashboard');

  const managedGroupIds = isSuperAdmin
    ? null
    : managedMemberships!.map(m => m.group_id);

  const groupQuery = adminClient.from('groups').select('id, name, discord_guild_id, icon_url').order('name');
  const { data: groups } = managedGroupIds
    ? await groupQuery.in('id', managedGroupIds)
    : await groupQuery;

  const groupIds = (groups ?? []).map(g => g.id);

  const [{ data: members }, { data: invites }, { data: allProfiles }] = await Promise.all([
    groupIds.length > 0
      ? adminClient
          .from('group_members')
    ...
    groupIds.length > 0
      ...
      : Promise.resolve({ data: [] }),
    adminClient.from('profiles').select('id, discord_username').order('discord_username'),
  ]);
                          ...
                          <div key={invite!.id} className="flex items-center justify-between px-5 py-2.5 gap-4">
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                              ...
                              <DestructiveButton
                                ...
                                action={revokeInvite.bind(null, group.id, invite!.id)}
                              />
                            </div>
                          </div>
                        );
                ...
                <form action={createInvite} className="px-5 py-4 space-y-3">
                  ...
                  <div className="flex gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Input name="max_uses" type="number" placeholder="Unlimited" min="1" className="w-28 text-xs py-1" />
                    </div>
