-- Enable fuzzystrmatch for levenshtein()
...
begin
  -- Determine whether the host said this sample was finished
  ...
  loop
    ...
    -- finish_type: only score if host said the whiskey was finished
    ...
    if r.scoring_type = 'exact' then
      if r.guess is null or r.guess = '' then
        ...
        update public.answers
        set points_earned = 3, fuzzy_flagged = false
        where id = r.answer_id;
      else
        -- Check fuzzy match (levenshtein <= 3) for non-boolean attributes
